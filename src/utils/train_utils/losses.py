...
def select_and_process_features(feature, mask, contrast_choice_num, mask_value):
    index = np.where(mask == mask_value)
    if len(index[0]) == 0:
        return None, None

    replace_id = len(index[0]) < contrast_choice_num
    selected_ids = np.random.choice(len(index[0]), size=contrast_choice_num, replace=replace_id)
...
def class_feature_vectors(feature, mask, args):
    with torch.no_grad():
        feature = feature.cpu().numpy()
        mask = mask.cpu().numpy()

        selected_feature_vector_1, _ = select_and_process_features(feature, mask, args.contrast_choice_num, 1)
        selected_feature_vector_2, _ = select_and_process_features(feature, mask, args.contrast_choice_num, 2)
        selected_feature_vector_3, _ = select_and_process_features(feature, mask, args.contrast_choice_num, 3)
...
def contrast_loss(args, feature1, feature2, gt, pseudo_gt):
    ...
    lb_class_1, lb_class_2, lb_class_3 = class_feature_vectors(lb_feature_net12, lb_gt_net12, args)
    ulb_class_1, ulb_class_2, ulb_class_3 = class_feature_vectors(ulb_feature_net12, ulb_gt_net12, args)
